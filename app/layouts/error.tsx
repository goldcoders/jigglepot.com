import { Link, MetaFunction, ThrownResponse } from "remix";


export const meta: MetaFunction = () => {
    return { title: "Error Page" };
};
function CatchError({ error, message }: { error: ThrownResponse; message: string }) {
    return (
        // add style 100vh
            ...
            <div className="min-h-full px-4 py-16 bg-white sm:px-6 sm:py-24 md:grid md:place-items-center lg:px-8">
                <div className="mx-auto max-w-max">
                    <main className="sm:flex">
                        <p className="text-4xl font-extrabold text-indigo-600 sm:text-5xl">{error.status}</p>
                        <div className="sm:ml-6">
                            <div className="sm:border-l sm:border-gray-200 sm:pl-6">
                                <h1 className="text-4xl font-extrabold tracking-tight text-gray-900 sm:text-5xl">{error.statusText}</h1>
